...
- Total Cash = Total Capital − buys + sells + income
- Realized P&L computed on demand by walking the transaction table per asset symbol (no snapshot); overview ROI uses Total Capital, while asset Return uses Total Buy Cost (buy outflows including fees)
